...
class Product extends Model
{
    ...
    protected $fillable = ['name', 'user_id', 'brand_id', 'category_id', 'created_by', 'slug', 'price', 'purchase_cost',
        ...
        'external_link', 'is_refundable', 'cash_on_delivery', 'attribute_sets', 'meta_image', 'colors',
        'selected_variants', 'selected_variants_ids', 'contact_info','status','has_variant', 'thumbnail', 'images', 'thumbnail_id', 'image_ids','stock_visibility'
    ];
    ...
    public function getTranslateAttribute()
    {
        ...
        if (blank($product_translation)):
            $product_translation = $this->hasMany(ProductLanguage::class)->where('lang', 'ka')->first();
        endif;
